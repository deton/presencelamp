...
+ 次の予約時刻が来たので、議論途中で会議室を空けないといけなくなった
...
## Raspberry Pi版
USB Wi-FiでのWPA2 Enterpriseへの接続が安定せず断念。
...
+ デバイス側プログラムをgolangで書いた理由は、golangの勉強のため。
Edisonだとnode.jsが入っているので、node.jsで書くのが楽かも。
...
+ ニッパを持ってないのでラジオペンチだけ使用。
  余分な足が短く切れなかったので見た目が悪い。
